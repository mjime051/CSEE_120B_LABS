...
/*
enum SCORE_STATES {SCORE_INIT, SCORE_INC, SCORE_DONE}SCORE_STATE;
char score;
char highScore;
int TickFct_Score(int SCORE_STATE){
	int next_state;
	switch(SCORE_STATE){
		case SCORE_INIT:
			if(gameFlag == 1){
				next_state = SCORE_INC;
			}
			else{
	...
	switch(SCORE_STATE){
		case SCORE_INIT:
			score = 0;
			highScore = eeprom_read_byte(0x01);
		break;
		case SCORE_INC:
			score++;
			PORTD = score;
		break;
		case SCORE_DONE:
			//display to screen the highscore
			PORTD = score;
		break;
	}
